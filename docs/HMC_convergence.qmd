...
## Gene-Level Perturbation Convergence Analysis
...
There is also a visualization function to examine the candidate convergence gene (`first_beta`) and selected convergence gene (`final_beta`, `second_pc` and `proj_direction`). Specifically, the `proj_direction` vector is the one used to calculate test staitstics for each fold. We expect certain consistency between the folds.
...
## Performance when No Convergence
...
## Perturbation Convergence on Gene Modules
